...
function LandingPage() {
  ...
  const vantaEffect = useRef(null); // Use a ref for vantaEffect to persist across renders
  ...
  useEffect(() => {
    if (!vantaEffect.current) {
      vantaEffect.current = HALO({
        ...
        amplitudeFactor: 2.5, // Adjust the halo's intensity
      });
    }
    ...
    return () => {
      if (vantaEffect.current) {
        ...
        vantaEffect.current = null; // Ensure cleanup on unmount
      }
    };
  }, []);
